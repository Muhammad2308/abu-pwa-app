...
import { FaUser, FaLock, FaSpinner, FaEnvelope, FaPhone, FaMapMarkerAlt, FaCity, FaBuilding, FaIdCard, FaArrowLeft, FaGraduationCap } from 'react-icons/fa';
import api, { donorsAPI, facultiesAPI } from '../services/api';
import abuLogo from '../assets/abu_logo.png';
import { GoogleSignInButton } from '../hooks/useGoogleAuth';

const Register = () => {
  const navigate = useNavigate();
  const { register, googleRegister, googleLogin, isAuthenticated, loading, user, checkSession } = useAuth();
  ...
  const handleAccountSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(result.message || 'Account created! Now complete your profile.');
        setStep('details');
  ...
  const handleDetailSubmit = async (e) => {
    ...
    try {
      // Build donor data object - only include fields that are relevant
      const donorData = {
      ...
      if (formData.other_name?.trim()) {
        donorData.other_name = formData.other_name.trim();
      }

      // Add alumni-specific fields ONLY if Alumni
      if (formData.donor_type === 'Alumni') {
        donorData.registration_number = formData.registration_number?.trim() || '';
        donorData.entry_year = formData.entry_year || null;
        donorData.graduation_year = formData.graduation_year || null;
        donorData.faculty_id = formData.faculty_id || null;
        donorData.department_id = formData.department_id || null;
      }

      // Add organization name ONLY if Organization/NGO
      if (formData.donor_type === 'Organization' || formData.donor_type === 'NGO') {
        donorData.organization_name = formData.organization_name?.trim() || null;
      }

      console.log('Creating donor profile:', donorData);
      ...
      if (response.data?.success) {
        ...
        donorId = response.data.data.id;
      } else if (response.data?.donor?.id) {
        donorId = response.data.donor.id;
      ...
      // Link donor to session - update session with donor_id
      const sessionId = localStorage.getItem('donor_session_id');
      if (sessionId) {
        try {
          await api.put(`/api/donor-sessions/${sessionId}`, {
            donor_id: parseInt(donorId, 10)
          });
          console.log('✅ Linked donor to session');
        } catch (linkError) {
          console.error('Error linking donor to session:', linkError);
        }
      }

      await checkSession();

      toast.success('Profile completed successfully! You can now donate.');
